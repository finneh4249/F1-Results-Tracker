...
class Menu(app_structures.BaseMenu):
    ...
    def view_constructor_standings(self):
        ...
        race = input()
        results = db_structures.get_standings_after_race("constructor",int(year), int(race))
        print(Fore.GREEN + "Getting Results...")
        app_structures.load(2)
        results_head = ["Position", "Constructor", "Points", "Positions Gained"]
        for r in races:
            if str(r[0]) == race:
                race_name = r[1]
                app_structures.clear()
        ...
        db_store_history.add_search(f"WCC Standings after {race_name}")
